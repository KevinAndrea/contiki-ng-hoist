...
PROCESS(multi_server_a_process, "Multi-Instance Server A");
AUTOSTART_PROCESSES(&multi_server_a_process);
/*---------------------------------------------------------------------------*/
static void
...
/*---------------------------------------------------------------------------*/
PROCESS_THREAD(multi_server_a_process, ev, data)
{
  PROCESS_BEGIN();

  /* Initialize DAG root */
  g_rpl_custom_instance = 0x1A;
